...
class HeatmiserProtocol(asyncio.Protocol):
    ...
    async def write_frame(self, frame):
        if self._lastwrite:
            start = datetime.now()
            log('debug', 'write delayed awaiting previous rx')
            while self._lastwrite and self._lastwrite + timedelta(seconds=2) > datetime.now():
                await asyncio.sleep(0.1)
            log('debug1', 'waited', (datetime.now() - start).seconds, 'seconds before writing')
...
class HeatmiserFrame:
    ...
    def set_bytes(self, index, value, length=1):
        ...
        log('debug1', "frame length:", len(self._data), "- target length:", flength, "- index:", index)
